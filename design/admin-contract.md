...
| `审核任务` | 技术审核、主编 | 作为新刊组稿流程的整期审核待办入口，只处理待审核期刊草稿 |
| `期刊管理` | 编辑、主编 | 按卷期管理新刊组稿上线、已发布内容维护和低频卷期设置 |
| `作者管理` | 编辑、主编 | 作者入库、修改、头像和合并 |
| `人员权限` | 主编 | 维护后台用户、角色和停用状态 |
| `操作日志` | 主编 | 查看关键后台操作记录 |

`期刊管理` 是期刊对象工作台，围绕选定卷期组织两条主工作流：

- 新刊组稿上线：创建期刊草稿、从候选稿件加入、编辑整期内容、整期预览、提交审核和发布已通过草稿。
- 已发布内容维护：选择该期已发布或已下线文章，直接修正、保存、下线、恢复和回滚；这条治理流程不进入整期技术审核，保存接口负责校验当前待保存内容。若界面提供手动检查入口，应表达为全站巡检，不能承诺已验证当前未保存编辑内容。

卷期创建、卷期 radar 更新等低频能力应与两条主工作流区分展示。已发布文章、已下线文章和历史快照都从某一期进入，不再作为全站平铺列表优先呈现。

`审核任务` 是新刊组稿上线流程中的技术审核待办入口，而不是第二套期刊管理。它只展示待审核的整期期刊草稿，并提供整期预览、内容检查、通过和退回；审核结束后，发布动作回到 `期刊管理` 的新刊组稿工作流。
...
`稿件池` 的单篇详情应呈现资产状态、组刊去向、发布去向和修改标记，不呈现单篇审核流程图；`期刊管理` 的整期草稿详情才呈现审核流程阶段，并把流转动作放在当前阶段附近。组刊和发布结果作为独立标记呈现，不应拉长审核流程图。期刊草稿处于待审核状态时，期刊管理只呈现状态和预览，不提供技术审核动作；通过或退回只能从审核任务执行。已发布内容维护不呈现整期审核流程或审核动作。不相关的审核动作不应长期作为禁用按钮挤占主工作区。期刊草稿的可加入稿件列表必须使用候选稿件 scope，不依赖稿件池当前视图。
