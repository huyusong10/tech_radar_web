...
| `POST /api/views/:vol` | `vol` | `{ vol, views }` | 卷期非法或不存在返回 `400`；资源忙返回 `503`；限流返回 `429` |
...
| `POST /api/admin/authors/merge` | `editor`、`chief_editor` | 合并两个正式作者并重写引用 |
...
| `POST /api/admin/users/:username/enable` | `chief_editor` | 重新启用后台操作者 |
...
- 阅读量写入仅面向存在的已发布卷期，服务端不得为格式不规范或不存在的卷期创建运行时计数。
