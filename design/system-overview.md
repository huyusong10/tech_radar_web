...
5. 为投稿者提供自助投稿、预览、状态查询与修改入口。
6. 为内网编辑团队提供后台管理入口，支持投稿初审、稿件池、期刊草稿、整期发布、作者维护和已发布内容治理。

## 模块职责

| 模块 | 唯一职责 | 对外依赖 | 对外输出 |
|------|----------|----------|----------|
| 页面壳层 | 组织当前期内容、归档导航、Best Practices、投稿列表、统计弹窗、搜索交互、投稿入口与图片导出 | HTTP 接口、静态 Markdown/图片资源、浏览器保存能力 | 用户可见页面、交互事件、投稿跳转与导出图片 |
...
| 后台管理界面 | 管理后台登录、投稿初审、稿件池、审核任务、期刊管理、作者管理、人员权限与操作日志 | 后台 API、浏览器文件导入能力 | 按用户工作流组织的后台页面 |
| 投稿者入口 | 提供文件导入、Markdown 预览、作者绑定、状态查询和修改 | 投稿 API、浏览器文件导入能力 | 投稿草稿、状态链接、投稿者可见反馈 |
| 后台内容写入层 | 将投稿者和后台操作转换为受控文件写入 | 后台私有数据仓、内容仓、作者主数据 | 投稿、稿件、期刊草稿、审核记录、作者更新、审计日志和发布后的正式投稿 |
...
```mermaid
flowchart TD
    A["编辑初审投稿"] --> B["归一化作者并创建稿件"]
    B --> C["加入期刊草稿并编排"]
    C --> D["整期期刊审核通过"]
    D --> E["主编发布期刊草稿"]
    E --> F["写入 published 内容仓"]
    F --> G["刷新归档、缓存和热更新"]
```
...
`投稿初审 -> 稿件池 -> 期刊草稿 -> 整期审核 -> 主编发布 -> 读者页热更新`

稳定承诺：

- 后台操作者与内容作者分离；后台权限不能从文章作者身份推导。
- 投稿允许临时作者；稿件池和已发布内容必须引用正式作者主数据。
- 稿件池是候选稿件资产库，不承载当前单篇审核流程；技术审核发生在整期期刊草稿上。
- 期刊草稿编排在选中具体草稿后从可入选稿件池中选择稿件，手动输入稿件 ID 只作为回退路径。
- 正式发布只能从已审核期刊草稿执行，且目标路径冲突时必须拒绝。
- 后台私有数据不对浏览器公开，只能通过后台 API 访问。
- 发布后文章、下线文章和历史快照从期刊管理中的某一期进入，而不是通过全站文章平铺列表作为主要入口。

### 投稿闭环

`投稿者提交 -> 编辑初审 -> 未接收可继续修改或接收入池 -> 期刊草稿 -> 整期审核 -> 发布`

稳定承诺：

- 读者页必须提供可直接到达 `/submit` 的投稿入口，不能要求投稿者先阅读后台文档或打开说明弹窗才能开始提交。
- 投稿者通过 token 链接访问自己的投稿，不进入后台权限体系。
- 投稿写入 `contents/admin/submissions/`，接收后创建 `contents/admin/manuscripts/`。
- 投稿者不决定目标卷期和发布目录；编辑组刊时在 `contents/admin/issue-drafts/` 中决定。
- 投稿者通过上传完整文件包提交新投稿或未接收投稿修改；接收入池后的稿件修改链接提供源码包下载、简单正文编辑和完整替换包上传。
- 编辑初审时先选择一种作者归一化方式，再填写对应字段；互斥路径不应在同一表单平面里同时展示。
- 未接收投稿的修改会增加 revision，并保留审核历史。
- 发布后投稿者状态链接可看到正式文章标识。
- 期刊草稿预览复用读者页完整界面，但内容来源切到后台期刊草稿预览 API；预览不记录访问量、点赞或搜索索引数据。
