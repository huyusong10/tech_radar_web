...
2. 以统一入口页呈现专题信息、Trending 条目、投稿内容、统计面板与检索结果。
...
| 模块 | 唯一职责 | 对外依赖 | 对外输出 |
|------|----------|----------|----------|
| 页面壳层 | 组织当前期内容、归档导航、投稿列表、统计弹窗与搜索交互 | HTTP 接口、静态 Markdown/图片资源 | 用户可见页面与交互事件 |
...
flowchart TD
    ...
    B --> C["加载共享配置与作者资料"]
    B --> D["读取 radar.md 与 contribution index.md"]
    D --> E["渲染专题头图、Trending、投稿卡片"]
    A --> F["增加当前卷阅读量"]
    A --> G["读取点赞总数与当前用户点赞状态"]
    G --> E
```
...
`卷期选择 -> 读取专题内容 -> 读取投稿目录 -> 拉取点赞/阅读统计 -> 渲染页面`
